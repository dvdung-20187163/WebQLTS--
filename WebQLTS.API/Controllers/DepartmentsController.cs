    ...
    public class DepartmentsController : BasesController<Department>
    {
        ...
        public IActionResult FilterDepartment(string? name, string? code)
        {
            try
            {
                ...
                if (code != null) whereConditions.Add($"(department_code LIKE \'%{code}%\') ");
                if (name != null) whereConditions.Add($" (department_name LIKE \'%{name}%\')");
                string whereClause = string.Join("AND", whereConditions);
                parameters.Add("d_Where", whereClause);

                // Khai báo tên procedure
                string storedProcedure = "Proc_department_Filter";
